...
public class ChannelCreate extends ListenerAdapter {
        ...
        public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
                ...
                if (args[0].equalsIgnoreCase(GuildData.getPrefix(event.getGuild().getId()) + "privatechannel")) {
                        if (Bools.isBlacklisted(event)) {
                                ...
                                if (Bools.isChannelSystemEnabled(event)) {
                                        if(args.length < 2) {
                                                ...
                                                nullArgs.setDescription("Correct Usage: `" + GuildData.getPrefix(event.getGuild().getId()) + "privatechannel <slowmode time> {-nsfw}` \nKey:\n<> | Required\n{} | Optional");
                                        ...
                                        if (categoryPrivate.size() < 1) {
                                                ...
                                                event.getGuild().getController().createTextChannel(event.getMember().getEffectiveName() + "s-private-text-channel").queue((channel) -> {
                                                                        ...
                                                                        channel.getManager().setTopic(event.getMember().getEffectiveName().toString() + "'s Private Text Channel \n\nOwner: " + event.getMember().getAsMention() + "\nInvited: None \n\nTo invite users the owner of the channel must run the command " + GuildData.getPrefix(event.getGuild().getId())  + "privateinvite <mention a user>");
