                        ...
                        if (Bools.isBlacklisted(event)) {
                                ...
                                if (Bools.isChannelSystemEnabled(event)) {
                                        if(args.length < 2) {
                                                ...
                                                nullArgs.setDescription("Correct Usage: `" + GuildData.getPrefix(event.getGuild()) + "privatechannel <slowmode time> {-nsfw}` \nKey:\n<> | Required\n{} | Optional");
