...
public class Help extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
        ...
        if (args[0].equalsIgnoreCase(Utils.getPrefix(event.getChannel()) + "help")) {
            ...
            if (Checks.isBlacklisted(event)) {
                ...
                owner.addField(Utils.getPrefix(event.getChannel()) + "announce <announcement>", "Will send a message to the owner of every server Quiver is in.", false);
                owner.addField(Utils.getPrefix(event.getChannel()) + "blacklist <user>", "Will blacklist a user from using Quiver", false);
                owner.addField(Utils.getPrefix(event.getChannel()) + "botinfo", "Will display all information on the bot. \nOptionally use: `--dm` to send in DM.", false);
                owner.addField(Utils.getPrefix(event.getChannel()) + "help", "Will show this embed. \nOptionally use: `--dm` to send in DM.", false);
                owner.addField(Utils.getPrefix(event.getChannel()) + "serverinfo", "Will send an embed with information about the server the command is used in. \nOptionally use: `--dm` to send in DM", false);
                owner.addField(Utils.getPrefix(event.getChannel()) + "userinfo", "Will send an embed with information about the user that ran the command \nOptionally use: `--dm` to send in DM", false);
                ...
                regular.addField(Utils.getPrefix(event.getChannel()) + "botinfo", "Will display information on the bot. \nOptionally use: `--dm` to send in DM", false);
                regular.addField(Utils.getPrefix(event.getChannel()) + "help" , "Will display this. \nOptionally use `--dm` to send in DM", false);
                regular.addField(Utils.getPrefix(event.getChannel()) + "serverinfo", "Will display information on the server the command is used in.", false);
                regular.addField(Utils.getPrefix(event.getChannel()) + "userinfo", "Will display information about the user that ran the command \nOptionally tag a member of the server to get information on them", false);
