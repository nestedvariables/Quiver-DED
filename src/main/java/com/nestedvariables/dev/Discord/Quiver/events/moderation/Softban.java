...
public class Softban extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
        ...
        if(args[0].equalsIgnoreCase(Utils.getPrefix(event.getGuild()) + "softban")){
            ...
            if (event.getMember().hasPermission(Permission.BAN_MEMBERS)) {
                if (args.length < 2) {
                    ...
                    nullReason.setFooter("Quiver Undefined Reason" , Utils.getSelfAvatar(event));
                    ...
                    ban.setFooter("Quiver SoftBan Report", Utils.getSelfAvatar(event));
